...
<%@page import="in.sterling.model.ConnectionProvider"%>
...
<body>
	...
	<%
	String passbookmessage = request.getAttribute("passbook") + "";
	if (passbookmessage.equalsIgnoreCase("Passbook downloaded successfully") && passbookmessage!=null) {
	%>
	<h4 style="color: green;"><%=passbookmessage%></h4>
	<%
	} %>
	<center>
		<%
		String downloadmsg = (String) request.getAttribute("downloadMessage");
		if (downloadmsg != null) {
		%>
		<%=downloadmsg%>
		<%
		...
		<table class="table table-hover" style="padding: 15px;">
			...
			<tbody>
				...
				<tr>
					<h3>Balance</h3>
					<td><p style="color: green; font-size: 20px;" /> <%=balance%>
						<%="Rs. " + r.getDouble("balance")%></td>
				</tr>
			</tbody>
			<%
			}
